              ...
              if (key.name === 0) {
                if (typeof self.currentContextParams()[key.name] !== 'undefined') {
                  if (app === 'filebrowser') {
                    ...
                    baseURL = baseURL.replace('*', encodeURIComponent(self.currentContextParams()[key.name]));
                  }
                } else {
                  baseURL = baseURL.replace('*', '');
                }
              } else {
                baseURL = baseURL.replace(':' + key.name, encodeURIComponent(self.currentContextParams()[key.name]));
              }
            });
          }
